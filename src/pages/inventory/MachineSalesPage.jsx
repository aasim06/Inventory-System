    ...
    <Stack spacing={3}>
      ...
      <Dialog open={printModalOpen} onClose={() => setPrintModalOpen(false)} maxWidth="md" fullWidth>
        ...
        <DialogTitle className="no-print" sx={{ fontWeight: 700, display: 'flex', justifyContent: 'space-between', alignItems: 'center', pb: 1 }}>
          ...
          <Chip label={printData?.paymentStatus || 'Paid'} color={printData?.paymentStatus === 'Paid' ? 'success' : 'warning'} size="small" />
        </DialogTitle>
          ...
          {printData && (() => {
            ...
            return (
              <Box id="printable-machine-invoice" sx={{ position: 'relative', overflow: 'hidden', p: { xs: 2, sm: 3 }, bgcolor: '#ffffff', color: '#1e293b', borderRadius: 1 }}>
                  ...
                  <Grid size={{ xs: 12, sm: 7 }}>
                    ...
                    {printData.customerPhone && (
                      <Typography variant="body2" color="textSecondary">
                        Contact: {printData.customerPhone}
                      </Typography>
                    )}
                    {printData.cityAddress && (
                      <Typography variant="caption" color="textSecondary" display="block">
                        Address: {printData.cityAddress}
                      </Typography>
                    )}
                  </Grid>
                ...
                {/* 🛒 Clean Simple Table (No Line Breaks in Headers) */}
                  ...
                  <Grid size={{ xs: 12, sm: 6 }}>
                    <Box sx={{ p: 1.5, bgcolor: '#ffffff', borderRadius: 1, border: '1px solid #e2e8f0', height: '100%' }}>
                      ...
                      <Typography variant="body2" color="textSecondary" sx={{ mt: 0.5 }}>
                        {printData.warrantyTerms || '1 Year Motor & Frame Free Service Warranty'}
                      </Typography>
                    </Box>
                  </Grid>
                  ...
                  <Grid size={{ xs: 12, sm: 6 }}>
                    <Box sx={{ p: 1.5, bgcolor: '#f8fafc', borderRadius: 1, border: '1px solid #cbd5e1', textAlign: 'right' }}>
                      ...
                      <Typography variant="subtitle1" fontWeight={700}>
                        Rs. {(printData.subTotal || printData.lineTotal || 0).toLocaleString()}
                      </Typography>

                      {(printData.discountAmount || 0) > 0 && (
                        <Typography variant="body2" color="error.main" sx={{ mt: 0.3 }}>
                          Discount: -Rs. {(printData.discountAmount || 0).toLocaleString()}
                        </Typography>
                      )}
                      ...
                      <Typography variant="h4" fontWeight={800} color="#166534">
                        Rs. {(printData.lineTotal || 0).toLocaleString()}
                      </Typography>

                      <Divider sx={{ my: 1 }} />

                      <Typography variant="body2" color="success.main" fontWeight={600}>
                        Paid: Rs. {(printData.paidAmount || 0).toLocaleString()}
                      </Typography>
                      <Typography variant="body2" color={(printData.balanceAmount || 0) > 0 ? 'error.main' : 'textSecondary'} fontWeight={600}>
                        Balance Due: Rs. {(printData.balanceAmount || 0).toLocaleString()}
                      </Typography>
                    </Box>
                  </Grid>
                </Grid>
                ...
                {/* ✍️ Clean Signatures */}
                <Grid container spacing={4} sx={{ mt: 3, pt: 1 }}>
                  <Grid size={{ xs: 6 }}>
                    <Box sx={{ borderTop: '1px solid #94a3b8', pt: 0.8, textAlign: 'center' }}>
